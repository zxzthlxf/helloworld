...
func recvNotification(r *readBuf) (*Notification, error) {
	bePid := r.int32()
	...
	return &Notification{bePid, channel, extra}, nil
}
...
func newDialListenerConn(_ Dialer, name string, c chan<- *Notification) (*ListenerConn, error) {
	cn, err := DialOpen(name)
	if err != nil {
		return nil, err
	}
...
func (l *ListenerConn) listenerConnLoop() (err error) {
	...
	for {
		...
		case 'A':
			...
			if err != nil {
				return fmt.Errorf("cannot recv notification: %w", err)
			}
			l.notificationChan <- not
		...
		case 'E':
			...
			if !l.setState(connStateExpectReadyForQuery) {
				return parseError(r, l.cn)
			}
			l.replyChan <- message{t, parseError(r, l.cn)}
...
func (l *ListenerConn) ExecSimpleQuery(q string) (executed bool, err error) {
	...
	if err != nil {
		...
		l.connectionLock.Unlock()
		if err := l.cn.c.Close(); err != nil {
			return false, fmt.Errorf("cannot close connect: %w", err)
		}
		return false, err
	}
